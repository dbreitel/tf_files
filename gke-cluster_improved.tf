...
# Create firewall rule for GCP health checks
resource "google_compute_firewall" "allow_health_checks" {
  name    = "allow-health-checks"
  network = "default"
  allow {
    protocol = "tcp"
    ports    = ["8080", "443", "30000-32767"]  # Include NodePort range and your service ports
  }
  source_ranges = ["35.191.0.0/16", "130.211.0.0/22"]  # GCP health checker IP ranges
  target_tags   = ["gke-node", "gke-gke001-my-node-pool-3091e681-node"]  # Target the node pool tags
  description   = "Allow health checks for GCP load balancers"
}

# Create firewall rule for service access
resource "google_compute_firewall" "allow_service_access" {
  name    = "allow-service-access"
  network = "default"
  allow {
    protocol = "tcp"
    ports    = ["8080", "443"]  # Your service ports
  }
  source_ranges = ["0.0.0.0/0"]  # Allow from any IP
  target_tags   = ["gke-node", "gke-gke001-my-node-pool-3091e681-node"]  # Target the node pool tags
  description   = "Allow external traffic to service ports"
}
